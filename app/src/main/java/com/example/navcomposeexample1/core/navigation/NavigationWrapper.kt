...
fun NavigationWrapper() {
    ...
    NavHost(navController = navController, startDestination = Login ){
        ...
        // by using this in home you can configure your deeplink such that if link match with this navigate to this UI
        composable<Home>(deepLinks = listOf(navDeepLink { uriPattern = "https://lodewijk.dev/home" }) )  {
            HomeScreen{ messageToDetail -> navController.navigate(Detail(name = messageToDetail)) }
        }
        // to prove this deep link go to Tools/App Links Assistant/ select test on device or emulator
        composable<Detail>(deepLinks = listOf(navDeepLink { uriPattern = "https://lodewijk.dev/detail/{name}" })) { backStackEntry ->
